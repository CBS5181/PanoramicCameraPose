...
)
{
  ...
  for (unsigned int iter = 0; iter < nIter && iter < num_max_iteration; ++iter)
  {
    // Get random samples
    if (bACRansacMode)
      UniformSample(sizeSample, random_generator, &vec_index, &vec_sample); // upper bound threshold is not provided;
    else
...
/**
    ...
    */
template<typename Kernel>
std::pair<double, double> Weighted_ACRANSAC
(
    const Kernel& kernel,
    std::vector<uint32_t>& vec_inliers,
...
)
{
    ...
    for (unsigned int iter = 0; iter < nIter && iter < num_max_iteration; ++iter)
    {
        ...
        if (bACRansacMode)
            WeightedSample(sizeSample, random_generator, &vec_index, &vec_sample, weights); // upper bound threshold is not provided;
        else
            UniformSample(sizeSample, nData, random_generator, &vec_sample);
