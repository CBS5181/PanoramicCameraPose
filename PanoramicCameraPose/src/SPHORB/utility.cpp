	...
	const Scalar& singlePointColor, int flags )
{
	...
	if( flags & static_cast<int>(DrawMatchesFlags::DRAW_OVER_OUTIMG) )
	{
	...
	// draw keypoints
	if( !(flags & static_cast<int>(DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS)) )
	{
		Mat _outImg1 = outImg( Rect(0, 0, img1.cols, img1.rows) );
		drawKeypoints( _outImg1, keypoints1, _outImg1, singlePointColor, static_cast<cv::DrawMatchesFlags>(flags + static_cast<int>(DrawMatchesFlags::DRAW_OVER_OUTIMG)) );

		Mat _outImg2 = outImg( Rect(0, img1.rows, img2.cols, img2.rows) );
		drawKeypoints( _outImg2, keypoints2, _outImg2, singlePointColor, static_cast<cv::DrawMatchesFlags>(flags + static_cast<int>(DrawMatchesFlags::DRAW_OVER_OUTIMG)) );
	}
}
...
{
	if(!vertical)
		cv::drawMatches(img1, keypoints1, img2, keypoints2, matches1to2, outImg, matchColor, singlePointColor, matchesMask, static_cast<cv::DrawMatchesFlags>(flags));
	else
	{
